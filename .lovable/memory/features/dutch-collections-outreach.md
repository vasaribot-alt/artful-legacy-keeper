...
- **Ask:** a single, concrete contribution of EUR 5 per contemporary work in their collection, which funds permanent archival preservation for artists. Alternative (low-pressure): institutional endorsement or a small joint pilot. Single ask, not two.
- **Never put the IBAN in outreach emails.** A bank account number in an unsolicited letter reads as a scam to corporate recipients. Giving is directed to the tracked Why GARF Matters page and /donate, where the bank details live.
- **Bank transfer on /donate:** interim path while Stripe is not yet activated (foundation's ABN AMRO account is open and working; only the bank cards are still awaited). Displayed: account name Global Artist Registry Foundation, IBAN NL93 ABNA 0156 3793 76, BIC ABNANL2A (not ABNANL2AXXX - the XXX is generic filler), Bank ABN AMRO Amsterdam, KvK 42024490. Payment reference = donor email so a receipt can be sent.
